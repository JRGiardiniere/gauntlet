...
export const makeReviewWorkspace = async (
  ...
  const bash = new Bash({
    ...
    // Bun has no such internal, so with the library default every bash call
    // fails. The library documents the layer as secondary; the overlay
    // filesystem and the confined command set are the ReviewWorkspace's
