...
day (best-effort — an unwritable `~/.gauntlet` re-probes), on a background
fiber that never fails a command and delays one by at most a second; `gauntlet upgrade` replaces the binary in place and touches
